    ...
    <div className="space-y-4 sm:space-y-5 lg:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0">
        <div>
          ...
          <p className="text-white/60 text-xs sm:text-sm">
            {isEditing ? t('general.subtitleEdit') : t('general.subtitleView')}
          </p>
        </div>
        ...
        {clientDetail && !isEditing && (
          <div className="bg-emerald-500/20 border border-emerald-400/30 rounded-lg sm:rounded-xl px-3 py-2 w-full sm:w-auto">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-emerald-100 text-xs sm:text-sm font-medium">{t('general.seniority')}</p>
          ...
          <div>
            ...
            {isEditing ? (
              <input
                ...
                placeholder={t('general.fieldNamePlaceholder')}
              ...
              <div className="px-3 sm:px-4 py-2.5 sm:py-3 bg-white/10 rounded-lg sm:rounded-xl border border-white/20 backdrop-blur-sm">
                <p className="text-white text-sm sm:text-base">{clientDetail?.client.nom_client || t('general.undefined')}</p>
              </div>
            )}
          </div>
            ...
            {isEditing ? (
              <div className="relative">
                ...
                <input
                  ...
                  placeholder={t('general.fieldPhonePlaceholder')}
              ...
              <div className="flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2.5 sm:py-3 bg-white/10 rounded-lg sm:rounded-xl border border-white/20 backdrop-blur-sm">
                ...
                <p className="text-white text-sm sm:text-base">{clientDetail?.client.tel_client || t('general.undefined')}</p>
              </div>
            )}
          </div>
            ...
            {isEditing ? (
              <div className="relative">
                ...
                <textarea
                  ...
                  placeholder={t('general.fieldAddressPlaceholder')}
                  rows={3}
              ...
              <div className="flex gap-2 sm:gap-3 px-3 sm:px-4 py-2.5 sm:py-3 bg-white/10 rounded-lg sm:rounded-xl border border-white/20 backdrop-blur-sm">
                ...
                <p className="text-white text-sm sm:text-base">{clientDetail?.client.adresse || t('general.undefinedF')}</p>
              </div>
            )}
          </div>
        </div>
        ...
        {isEditing && (
          <div className="flex items-center justify-end gap-2 sm:gap-3 mt-4 sm:mt-6 pt-3 sm:pt-4 border-t border-white/20">
            ...
            >
              ...
              <span className="hidden sm:inline">{t('general.cancel')}</span>
            </button>
          </div>
        )}
      </div>
      ...
      {!clientDetail && !isEditing && !isLoading && (
        <div className="text-center py-8 bg-white/5 rounded-2xl border border-white/20">
          ...
          <p className="text-white/60">{t('general.noInfo')}</p>
          <button
            onClick={() => setIsEditing(true)}
