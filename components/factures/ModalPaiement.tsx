...
export function ModalPaiement({
  ...
  // (secondes incluses : deux tranches CASH dans la même minute ne doivent pas
  // produire la même référence)
                ...
                {resteApresTranche !== null && !success && (
                  <div className={`flex items-center gap-2 bg-emerald-50 border border-emerald-200 rounded-lg ${isCompact ? 'p-2' : 'p-3'}`}>
                    ...
                    <p className={`text-emerald-700 font-medium ${styles.subtitle}`}>
                      {t('payment.trancheRecorded', { amount: formatNumber(resteApresTranche, locale) })}
                    </p>
                  </div>
                )}
