...
import { PartenaireStatsVentes, PartenaireStatsVentesParams } from '@/types/partenaire.types';

// ========================================
// Props
// ========================================
interface PartenaireVentesTabProps {
  idPartenaire: number;
}
...
// Composant Principal
// ========================================
export default function PartenaireVentesTab({ idPartenaire }: PartenaireVentesTabProps) {
  // États
  const [loading, setLoading] = useState(true);
  const [stats, setStats] = useState<PartenaireStatsVentes | null>(null);
  ...
  const loadStats = useCallback(async () => {
    if (!idPartenaire) return;

    setLoading(true);
    setError(null);

    try {
      const params: PartenaireStatsVentesParams = {
        annee,
        mois,
        limit_top: 10
      };

      const response = await partenaireService.getStatsVentes(idPartenaire, params);
      ...
      console.error('Erreur chargement stats ventes partenaire:', err);
      setError('Impossible de charger les statistiques de ventes');
    } finally {
      setLoading(false);
    }
  }, [idPartenaire, annee, mois]);
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-20">
        ...
        <span className="ml-3 text-gray-400">Chargement des statistiques de ventes...</span>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-white flex items-center gap-2">
            ...
            Ventes de mes Structures
          </h2>
          <p className="text-gray-400 text-sm">{periode?.label || `Année ${annee}`}</p>
        </div>
      ...
      {showFilters && (
        ...
        >
          ...
          <div>
            ...
            <select
              value={mois || ''}
              onChange={(e) => setMois(e.target.value ? Number(e.target.value) : null)}
              className="px-3 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white text-sm"
            >
                ...
                return (
                  <div key={idx} className="flex flex-col items-center min-w-[45px] group relative">
                    ...
                    <div className="relative w-full h-40 flex items-end justify-center">
                      <div
                        className="w-8 bg-gradient-to-t from-orange-600 to-orange-400 rounded-t transition-all duration-300 group-hover:from-orange-500 group-hover:to-orange-300 shadow-lg shadow-orange-500/20"
                        style={{ height: `${barHeight}%` }}
                      />
                    </div>
                    {/* Label mois/jour */}
                    <span className="text-xs text-gray-400 mt-1 font-medium">
                      {item.label || item.jour}
                    </span>
                  </div>
                );
              })}
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="bg-gray-800/50 rounded-xl p-4 border border-gray-700">
          ...
          <div className="space-y-3">
            {top_produits && top_produits.length > 0 ? (
              ...
              <p className="text-gray-500 text-center py-4">Aucun produit vendu</p>
            )}
          </div>
        </div>
        ...
        <div className="bg-gray-800/50 rounded-xl p-4 border border-gray-700">
          ...
          <div className="space-y-3">
            {par_categorie && par_categorie.length > 0 ? (
      ...
      <div className="bg-gray-800/50 rounded-xl p-4 border border-gray-700">
        <h3 className="text-lg font-semibold mb-4 flex items-center gap-2">
          ...
          Classement de mes Structures
          <span className="text-xs text-gray-400 font-normal ml-2">part du CA total</span>
        </h3>
        <div className="space-y-3">
          {par_structure && par_structure.length > 0 ? (
            ...
            <p className="text-gray-500 text-center py-4">Aucune structure avec ventes</p>
          )}
        </div>
      </div>
    </div>
